...
SELECT 
    last_name AS 'unique last names'
FROM
    actor
GROUP BY last_name
...
SELECT 
    last_name 'duplicate last names'
FROM
    actor
GROUP BY last_name
HAVING COUNT(last_name) > 1;
...
SELECT 
    staff_id, COUNT(staff_id) AS 'number of rentals'
FROM
    rental
GROUP BY staff_id;
...
SELECT 
    release_year, COUNT(film_id) AS 'releases this year'
FROM
    film
...
SELECT 
    rating, COUNT(film_id) AS 'movies'
FROM
    film
GROUP BY rating;
...
SELECT 
    rating,
    ROUND(SUM(length) / COUNT(length), 2) AS 'mean length'
FROM
    film
GROUP BY rating;
